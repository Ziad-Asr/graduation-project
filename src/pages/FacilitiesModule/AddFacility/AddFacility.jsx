...
const AddFacility = () => {
  ...
  const validateForm = () => {
    ...
    if (
      ...
      newErrors.name = "Name must be between 1 and 10 characters";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await dispatch(addFacility(formDataToSend)).unwrap();

      if (response?.status === 200) {
        toast.success("Facility added successfully!");
        navigate("/facilities");
      } else {
        toast.error(response || "Failed to add facility", {
          position: "top-right",
          autoClose: 2000,
